param([switch]$RedproPreset, [switch]$RedproLaunch, [string]$SelectedAdapter)
<# 
.NAME
    Tweaking Adapter
#>
...
0..63 | ForEach-Object {[void] $cb_rssmaxproc.Items.Add([string]$_)}
...
1..64 | ForEach-Object {[void] $cb_rssmaxprocs.Items.Add([string]$_)}
...
function ApplyInterfaceSettings{
    ...
    if ($cb_DirectedMacWolPattern.Text -eq (Get-NetIPInterface -InterfaceAlias $NetConnectionID -AddressFamily $Global:AddressFamily | Select-Object -Expand DirectedMacWolPattern)){
        Write-Host "DirectedMacWolPattern is same, skipping."  -ForegroundColor green}
            ...
            Set-NetIPInterface -InterfaceAlias $NetConnectionID -AddressFamily $Global:AddressFamily -DirectedMacWolPattern $cb_DirectedMacWolPattern.Text
        }
...
function applyall{
    Set-ItemProperty -Path $TCPIP_RegPath -Name RssBaseCpu -Value ([int]$cb_tcpiprssbasecpu.Text) -Type DWord -Force
    Set-ItemProperty -Path $NDIS_RegPath -Name RssBaseCpu -Value ([int]$cb_ndisrssbasecpu.Text) -Type DWord -Force
    if ($btn_InterruptApply.Enabled) { HWSettings }
...
# Redpro preset fills controls only; the existing Apply actions commit changes.
function Set-RedproPreset {
    if ($cb_AdapterNamesCombo.SelectedIndex -lt 0) {
        [Windows.Forms.MessageBox]::Show('เลือก Adapter ก่อน แล้วกดโหลดค่าอัตโนมัติ', 'Redpro') | Out-Null
        return
    }
    $threads = [int](Get-CimInstance Win32_ComputerSystem).NumberOfLogicalProcessors
    if ($threads -lt 1) { $threads = [Environment]::ProcessorCount }
    $values = @{
        cb_rss_onoff='Enable'; cb_rssqueues='4'; cb_rssprofile='ClosestStatic'; cb_rssbaseproc='0'
        cb_rssmaxproc=[string]([Math]::Min(63,$threads-1)); cb_rssmaxprocs=[string]([Math]::Min(64,$threads))
        cb_DisablePortScaling='Enabled'; cb_ManyCoreScaling='Disabled'
        cb_tcpiprssbasecpu='0'; cb_ndisrssbasecpu='0'; cb_MsiMode='Enabled'; cb_InterruptPriority='Undefined'; cb_DevicePolicy='AllCloseProcessors'
        cb_osrss='Enabled'; cb_osrsc='Disabled'; cb_oschimney='Disabled'; cb_ostaskoff='Disabled'; cb_osntd='Disabled'; cb_osntdais='Blocked'; cb_ospcf='Disabled'
        cb_AdvertiseDefaultRoute='Enabled'; cb_Advertising='Disabled'; cb_AutomaticMetric='Enabled'; cb_ClampMss='Disabled'; cb_DirectedMacWolPattern='Disabled'; cb_EcnMarking='AppDecide'; cb_ForceArpNdWolPattern='Disabled'; cb_Forwarding='Disabled'; cb_IgnoreDefaultRoutes='Disabled'; cb_ManagedAddressConfiguration='Enabled'; cb_NeighborDiscoverySupported='Yes'; cb_NeighborUnreachabilityDetection='Enabled'; cb_OtherStatefulConfiguration='Enabled'; cb_RouterDiscovery='ControlledByDHCP'; cb_Store='ActiveStore'; cb_WeakHostReceive='Disabled'; cb_WeakHostSend='Disabled'
        tb_CurrentHopLimit='0'; tb_BaseReachableTime='30000'; tb_RetransmitTime='500'; tb_ReachableTime='30000'; tb_DadRetransmitTime='1000'; tb_DadTransmits='3'; tb_NlMtu='1500'
        cb_ReceiveBuffers='512'; cb_TransmitBuffers='1024'; cb_InterruptModerationRate='0'; cb_PacketDirect='Disabled'; cb_EnableCoalesce='Disabled'; cb_CoalesceBufferSize='0'; cb_EnableUdpTxScaling='Enabled'
        cb_EnablePME='Disabled'; cb_EnableDynamicPowerGating='Disabled'; cb_EnableConnectedPowerGating='Disabled'; cb_AutoPowerSaveModeEnabled='Disabled'; cb_NicAutoPowerSaver='Disabled'; cb_DisableDelayedPowerUp='Disabled'; cb_ReduceSpeedOnPowerDown='Disabled'
    }
    $cb_IPv6.Checked = $false
    $cb_IPv4.Checked = $true
    ...
    foreach ($name in $values.Keys) {
        ...
        if (-not $control.Enabled) { $skipped.Add($name); continue }
        $value = $values[$name]
        if ($control -is [Windows.Forms.ComboBox] -and $control.DropDownStyle -eq 'DropDownList' -and -not $control.Items.Contains($value)) {
            $skipped.Add($name); continue
        }
        $control.Text = $value
    }
    foreach ($name in @('cb_flowcontrol','cb_IPChecksumOffloadIPv4','cb_TCPChecksumOffloadIPv4','cb_TCPChecksumOffloadIPv6','cb_UDPChecksumOffloadIPv4','cb_UDPChecksumOffloadIPv6','cb_LsoV1IPv4','cb_LsoV2IPv4','cb_LsoV2IPv6','cb_PMARPOffload','cb_PMNSOffload','cb_InterruptModeration','cb_PriorityVLANTag')) {
        $control = Get-Variable $name -ValueOnly
        $index = if ($name -eq 'cb_PriorityVLANTag') { 3 } else { 0 }
        if ($control.Enabled -and $control.Items.Count -gt $index) { $control.SelectedIndex = $index }
        else { $skipped.Add($name) }
    }
    $message = "โหลดค่าลงช่องแล้ว (ยังไม่ได้ Apply) • CPU $threads threads`nตรวจค่าแล้วกด Apply All จากนั้นรีสตาร์ตเครื่องด้วยตนเอง`nส่วน Tweaks ไม่ถูกเปลี่ยน"
    if ($skipped.Count) { $message += "`nค่าที่อุปกรณ์ไม่รองรับ/แก้ไขไม่ได้: " + ($skipped -join ', ') }
    [Windows.Forms.MessageBox]::Show($message,'Redpro Network Preset') | Out-Null
}
$btnRedproPreset = New-Object Windows.Forms.Button
$btnRedproPreset.Text = 'โหลดค่าอัตโนมัติ'
$btnRedproPreset.SetBounds(750,12,190,32)
$btnRedproPreset.FlatStyle = 'Flat'
$btnRedproPreset.BackColor = [Drawing.ColorTranslator]::FromHtml('#7C3AED')
$btnRedproPreset.ForeColor = [Drawing.Color]::White
$btnRedproPreset.Font = New-Object Drawing.Font('Leelawadee UI',10)
$btnRedproPreset.Add_Click({ Set-RedproPreset })
$Form.Controls.Add($btnRedproPreset)
$Form.AutoScroll = $true
$Form.StartPosition = 'CenterScreen'
if ($RedproPreset -or $RedproLaunch -or $SelectedAdapter) {
    $Form.Add_Shown({
        if ($SelectedAdapter) {
            $adapterIndex = $cb_AdapterNamesCombo.FindStringExact($SelectedAdapter)
            if ($adapterIndex -ge 0) {
                $cb_AdapterNamesCombo.SelectedIndex = $adapterIndex
            } else {
                [Windows.Forms.MessageBox]::Show("ไม่พบ Adapter ที่เลือก: $SelectedAdapter", 'Redpro') | Out-Null
            }
        }
        if ($RedproPreset) { Set-RedproPreset }
        if ($RedproLaunch) {
        $Form.Activate()
        $Form.BringToFront()
        $Form.TopMost = $false
