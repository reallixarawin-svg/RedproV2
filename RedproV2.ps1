...
# Network and performance controls use the existing theme.
[xml]$networkUi = $xaml
...
$view.InnerXml = @"
<ScrollViewer xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" VerticalScrollBarVisibility="Auto"><StackPanel Margin="0,0,16,0">
<TextBlock Text="Network / Latency / FPS" Foreground="White" FontSize="30" FontWeight="Bold" Margin="0,0,0,12"/>
<TextBlock Text="สำรองค่าก่อนปรับ • ผลลัพธ์ขึ้นกับเครื่องและเครือข่าย • รีสตาร์ตหลังปรับการ์ดเน็ต" Foreground="#AEB9CC" TextWrapping="Wrap" Margin="0,0,0,16"/>
<Button x:Name="BtnSelectAdapter" Content="เลือก Network Adapter" Style="{StaticResource SecondaryButton}" Height="40"/>
<TextBlock x:Name="TxtSelectedAdapter" Text="ยังไม่ได้เลือก Adapter" Foreground="#B79AFF" Margin="0,10,0,14"/>
<Button x:Name="BtnNetTweak" Content="NETWORK — TCP Auto-Tuning Normal + RSS" Style="{StaticResource PremiumButton}" Height="44" Margin="0,0,0,10"/>
<Button x:Name="BtnAdapterPreset" Content="LOW LATENCY — ปิด Interrupt Moderation / EEE" Style="{StaticResource PremiumButton}" Height="44" Margin="0,0,0,10"/>
<TextBlock Text="Low Latency ปรับเฉพาะค่าที่ไดรเวอร์รองรับ อาจใช้ CPU เพิ่มหรือทำให้ FPS ลดลง ควรทดสอบก่อนและหลัง ไม่เปลี่ยน DNS, MTU หรือบังคับความเร็วลิงก์" Foreground="#AEB9CC" TextWrapping="Wrap" Margin="0,0,0,12"/>
<UniformGrid Columns="2"><Button x:Name="BtnNetFlush" Content="Flush DNS cache" Style="{StaticResource SecondaryButton}" Height="40" Margin="0,0,5,0"/><Button x:Name="BtnNetRestore" Content="คืนค่าเน็ตจาก Backup" Style="{StaticResource SecondaryButton}" Height="40" Margin="5,0,0,0"/></UniformGrid>
<TextBlock Text="FPS / GAME LATENCY" Foreground="White" FontSize="22" Margin="0,24,0,12"/>
<Button x:Name="BtnFpsBoost" Content="FPS BOOST — High Performance + ปิด Game Capture" Style="{StaticResource PremiumButton}" Height="44" Margin="0,0,0,10"/>
<TextBlock Text="ใช้พลังงานและความร้อนเพิ่ม • ถ้าเครื่องไม่มีแผน High Performance จะรายงานข้อผิดพลาด • เปิด Game Mode และเลือก GPU ประสิทธิภาพสูงให้เกมใน Settings" Foreground="#AEB9CC" TextWrapping="Wrap" Margin="0,0,0,12"/>
<UniformGrid Columns="2"><Button x:Name="BtnGameSettings" Content="Game Mode Settings" Style="{StaticResource SecondaryButton}" Height="40" Margin="0,0,5,0"/><Button x:Name="BtnGraphicsSettings" Content="Graphics / Windowed Optimizations" Style="{StaticResource SecondaryButton}" Height="40" Margin="5,0,0,0"/></UniformGrid>
<Button x:Name="BtnFpsRestore" Content="คืนค่า FPS จาก Backup" Style="{StaticResource SecondaryButton}" Height="40" Margin="0,10,0,12"/>
</StackPanel></ScrollViewer>
"@

# Add an in-app command log view and its sidebar menu item.
$restoreNav = $networkUi.SelectSingleNode('//*[@x:Name="BtnNavRestore"]',$ns)
$logsNav = $restoreNav.CloneNode($true)
$logsNav.SetAttribute('Name','http://schemas.microsoft.com/winfx/2006/xaml','BtnNavLogs')
$logsNav.InnerXml = '<TextBlock xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" Text="☷  Logs" />'
[void]$restoreNav.ParentNode.InsertAfter($logsNav,$restoreNav)
$logsView = $networkUi.CreateDocumentFragment()
$logsView.InnerXml = @"
<Grid xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" x:Name="ViewLogs" Visibility="Collapsed">
  <Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="*"/></Grid.RowDefinitions>
  <StackPanel Margin="0,0,0,22"><TextBlock Text="Command Logs" Foreground="#F2F4F8" FontSize="30" FontWeight="Bold"/><TextBlock Text="คำสั่งที่ RedproV2 รันล่าสุด" Foreground="#AEB9CC" FontSize="15" Margin="0,6,0,0"/></StackPanel>
  <Border Grid.Row="1" Background="{StaticResource CardGradient}" CornerRadius="12" Padding="22" BorderThickness="1" BorderBrush="#2C3445"><Grid><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="*"/></Grid.RowDefinitions><DockPanel Margin="0,0,0,16"><Button x:Name="BtnLogRefresh" Content="รีเฟรช" Style="{StaticResource PremiumButton}" Height="38" Width="110" DockPanel.Dock="Right"/><TextBlock Text="BUTTON HISTORY" Foreground="#B79AFF" FontSize="12" FontWeight="Bold" VerticalAlignment="Center"/></DockPanel><RichTextBox x:Name="TxtLogViewer" Grid.Row="1" Background="#10131C" Foreground="#D8E1F0" BorderBrush="#30394C" BorderThickness="1" Padding="14" FontFamily="Consolas" FontSize="12" IsReadOnly="True" IsDocumentEnabled="False" VerticalScrollBarVisibility="Auto"/></Grid></Border>
</Grid>
"@
[void]$view.ParentNode.AppendChild($logsView)
...
$views = @("ViewDashboard", "ViewCPU", "ViewInput", "ViewNetwork", "ViewFiveM", "ViewPower", "ViewTools", "ViewRestore", "ViewLogs")
$navButtons = @("BtnNavDashboard", "BtnNavCPU", "BtnNavInput", "BtnNavNetwork", "BtnNavFiveM", "BtnNavPower", "BtnNavTools", "BtnNavRestore", "BtnNavLogs")

# LOG FUNCTION — kept only for the current app session.
$script:activityLog = New-Object 'System.Collections.Generic.List[string]'
function Write-Log {
    param([string]$Message)
    $script:activityLog.Add(('{0:HH:mm:ss}  {1}' -f (Get-Date), $Message))
    if ($script:activityLog.Count -gt 500) { $script:activityLog.RemoveAt(0) }
}

function Get-ButtonLabel {
    param([System.Windows.Controls.Button]$Button)
    if ($Button.Content -is [string]) { return $Button.Content.Trim() }
    if ($Button.Content -is [System.Windows.Controls.TextBlock]) { return $Button.Content.Text.Trim() }
    if ($Button.Content -is [System.Windows.Controls.StackPanel]) {
        $text = @($Button.Content.Children | Where-Object { $_ -is [System.Windows.Controls.TextBlock] -and $_.FontFamily -notmatch 'Segoe MDL2 Assets' } | ForEach-Object { $_.Text.Trim() }) -join ' '
        if ($text) { return $text }
    }
    return $Button.Name
}
...
function Update-LogViewer {
    $viewer = $window.FindName('TxtLogViewer')
    $viewer.Document.Blocks.Clear()
    try {
        $lines = @($script:activityLog | Where-Object { $_ -match 'BUTTON \[' })
        if ($lines.Count -eq 0) { $lines = @('ยังไม่มีการกดปุ่มที่บันทึกไว้') }
        foreach ($line in $lines) {
            $run = New-Object Windows.Documents.Run($line)
            if ($line -match 'SUCCESS') { $run.Foreground = [Windows.Media.BrushConverter]::new().ConvertFromString('#86EFAC') }
            elseif ($line -match 'FAILED') { $run.Foreground = [Windows.Media.BrushConverter]::new().ConvertFromString('#FDA4AF') }
            else { $run.Foreground = [Windows.Media.BrushConverter]::new().ConvertFromString('#D8E1F0') }
            $paragraph = New-Object Windows.Documents.Paragraph
            $paragraph.Margin = '0,0,0,6'
            [void]$paragraph.Inlines.Add($run)
            [void]$viewer.Document.Blocks.Add($paragraph)
        }
        $viewer.ScrollToEnd()
    } catch {
        $viewer.Document.Blocks.Clear()
        [void]$viewer.Document.Blocks.Add((New-Object Windows.Documents.Paragraph (New-Object Windows.Documents.Run('อ่าน log ไม่สำเร็จ: ' + $_.Exception.Message))))
    }
}
...
$window.FindName("BtnNavLogs").Add_Click({ Switch-View "ViewLogs" "BtnNavLogs"; Update-LogViewer })
$window.FindName("BtnLogRefresh").Add_Click({ Update-LogViewer })
...
function Run-Tweak {
    ...
    $buttonLabel = Get-ButtonLabel $Btn
    Write-Log "BUTTON [$buttonLabel] | STARTED"
    $Btn.IsEnabled = $false
    ...
    try {
        ...
        $succeeded = $false
        [Windows.MessageBox]::Show($_.Exception.Message + "`nIf partially applied, use the matching profile Restore button.", "Profile failed") | Out-Null
        Write-Log "BUTTON [$buttonLabel] | FAILED | $($_.Exception.Message)"
    }

    $statusText = if ($succeeded) { 'SUCCESS!' } else { 'FAILED!' }
    if ($txtBlock) {
        $txtBlock.Text = $statusText
    } elseif ($txt -is [System.Windows.Controls.TextBlock]) {
        $txt.Text = $statusText
    } else {
        $Btn.Content = $statusText
    }
    ...
    $Btn.IsEnabled = $true
    if ($succeeded) { Write-Log "BUTTON [$buttonLabel] | SUCCESS" }
}
...
$window.FindName("BtnApplyRec").Add_Click({
    Run-Tweak -Btn $window.FindName("BtnApplyRec") -LoadingText "APPLYING FPS..." -Action { Set-FpsProfile }
})
# CPU ACTIONS
...
$script:selectedNetworkAdapter = $null
function Select-RedproNetworkAdapter {
    try {
        $adapters = @(Get-NetAdapter -Physical -ErrorAction Stop | Where-Object { $_.Status -eq 'Up' } | Sort-Object -Property InterfaceDescription)
        if ($adapters.Count -eq 0) { $adapters = @(Get-NetAdapter -Physical -ErrorAction Stop | Sort-Object -Property InterfaceDescription) }
        if ($adapters.Count -eq 0) { throw 'ไม่พบ Network Adapter' }

        $picker = New-Object Windows.Window
        $picker.Title = 'เลือก Network Adapter'
        $picker.Width = 480; $picker.Height = 190
        $picker.WindowStartupLocation = 'CenterOwner'; $picker.Owner = $window
        $picker.Background = [Windows.Media.Brushes]::White
        $layout = New-Object Windows.Controls.StackPanel
        $layout.Margin = '20'
        $layout.Children.Add((New-Object Windows.Controls.TextBlock -Property @{ Text = 'เลือกการ์ดแลนที่จะปรับ'; FontSize = 16; Margin = '0,0,0,12' })) | Out-Null
        $combo = New-Object Windows.Controls.ComboBox
        $combo.Height = 32; $combo.FontSize = 14
        foreach ($adapter in $adapters) { [void]$combo.Items.Add($adapter.InterfaceDescription) }
        if ($script:selectedNetworkAdapter) { $combo.SelectedItem = $script:selectedNetworkAdapter }
        if ($combo.SelectedIndex -lt 0) { $combo.SelectedIndex = 0 }
        $layout.Children.Add($combo) | Out-Null
        $choose = New-Object Windows.Controls.Button
        $choose.Content = 'เลือก'; $choose.Height = 34; $choose.Width = 110
        $choose.HorizontalAlignment = 'Right'; $choose.Margin = '0,14,0,0'
        $choose.Add_Click({ $script:selectedNetworkAdapter = [string]$combo.SelectedItem; $picker.DialogResult = $true })
        $layout.Children.Add($choose) | Out-Null
        $picker.Content = $layout
        if ($picker.ShowDialog()) {
            $window.FindName('TxtSelectedAdapter').Text = $script:selectedNetworkAdapter
            Write-Log "BUTTON [เลือก Adapter: $script:selectedNetworkAdapter] | SUCCESS"
            return $true
        }
    } catch {
        Write-Log "BUTTON [เลือก Adapter] | FAILED | $($_.Exception.Message)"
        [Windows.MessageBox]::Show($_.Exception.Message, 'เลือก Network Adapter') | Out-Null
    }
    return $false
}

# Performance profiles: changes happen only after an explicit button click.
$script:profileRoot = Join-Path $env:LOCALAPPDATA 'RedproV2\ProfileBackups'
function Invoke-ProfileCommand {
    param([string]$File, [string[]]$Arguments)
    $output = & $File @Arguments 2>&1
    if ($LASTEXITCODE -ne 0) { throw "$File failed ($LASTEXITCODE): $($output -join ' ')" }
    return $output
}
function Get-ProfileAdapter {
    if (-not $script:selectedNetworkAdapter) { throw 'Select an adapter first.' }
    $a = @(Get-NetAdapter -Physical -ErrorAction Stop | Where-Object InterfaceDescription -eq $script:selectedNetworkAdapter)
    if ($a.Count -ne 1) { throw 'Adapter selection is missing or ambiguous. Select again.' }
    return $a[0]
}
function Save-ProfileState {
    param($State, [string]$Path)
    [IO.Directory]::CreateDirectory($script:profileRoot) | Out-Null
    $temp = $Path + '.tmp'
    $State | Export-Clixml -LiteralPath $temp -ErrorAction Stop
    Move-Item -LiteralPath $temp -Destination $Path -Force -ErrorAction Stop
}
function Set-NetworkProfile {
    param([switch]$LowLatency)
    $a = Get-ProfileAdapter
    $path = Join-Path $script:profileRoot ('network-' + $a.InterfaceGuid + '.xml')
    $props = @(Get-NetAdapterAdvancedProperty -Name $a.Name -AllProperties -ErrorAction Stop)
    if (-not (Test-Path -LiteralPath $path)) {
        $tcp = (Get-NetTCPSetting -SettingName Internet -ErrorAction Stop).AutoTuningLevelLocal.ToString().ToLowerInvariant()
        if ($tcp -notin @('disabled','highlyrestricted','restricted','normal','experimental')) { throw "Unsupported TCP state: $tcp" }
        # TCP tuning is global; preserve one baseline across all adapter profiles.
        $globalPath = Join-Path $script:profileRoot 'tcp.xml'
        if (-not (Test-Path -LiteralPath $globalPath)) { Save-ProfileState @{ AutoTune = $tcp } $globalPath }
        Save-ProfileState @{ Guid = [string]$a.InterfaceGuid; Properties = @($props | Where-Object RegistryKeyword -in @('*RSS','*InterruptModeration','*EEE')) } $path
    }
    Invoke-ProfileCommand netsh.exe @('int','tcp','set','global','autotuninglevel=normal') | Out-Null
    $targets = @{ '*RSS' = '1' }
    if ($LowLatency) { $targets['*InterruptModeration'] = '0'; $targets['*EEE'] = '0' }
    foreach ($key in $targets.Keys) {
        $p = $props | Where-Object RegistryKeyword -eq $key | Select-Object -First 1
        if (-not $p) { Write-Log "BUTTON [Network] | SKIPPED | $key unsupported"; continue }
        if (@($p.ValidRegistryValues) -notcontains $targets[$key]) { Write-Log "BUTTON [Network] | SKIPPED | $key value unsupported"; continue }
        Set-NetAdapterAdvancedProperty -Name $a.Name -RegistryKeyword $key -RegistryValue $targets[$key] -NoRestart -ErrorAction Stop
    }
    [Windows.MessageBox]::Show('Saved. Restart Windows to apply adapter settings. Compare ping, jitter and FPS before keeping Low Latency; it can increase CPU load.', 'Network profile') | Out-Null
}
function Restore-NetworkProfile {
    $a = Get-ProfileAdapter
    $path = Join-Path $script:profileRoot ('network-' + $a.InterfaceGuid + '.xml')
    if (-not (Test-Path -LiteralPath $path)) { throw 'No saved baseline for this adapter.' }
    $state = Import-Clixml -LiteralPath $path -ErrorAction Stop
    foreach ($p in $state.Properties) {
        Set-NetAdapterAdvancedProperty -Name $a.Name -RegistryKeyword $p.RegistryKeyword -RegistryValue $p.RegistryValue -NoRestart -ErrorAction Stop
    }
    $tcp = Import-Clixml -LiteralPath (Join-Path $script:profileRoot 'tcp.xml') -ErrorAction Stop
    Invoke-ProfileCommand netsh.exe @('int','tcp','set','global',('autotuninglevel=' + $tcp.AutoTune)) | Out-Null
    [Windows.MessageBox]::Show('Original adapter and global TCP values restored. Restart Windows. Other adapters keep their own settings.', 'Restore') | Out-Null
}
function Set-FpsProfile {
    $path = Join-Path $script:profileRoot 'fps.xml'
    $entries = @(
    ...
    if (-not (Test-Path -LiteralPath $path)) {
        $plan = (Invoke-ProfileCommand powercfg.exe @('/getactivescheme')) -join ' '
        if ($plan -notmatch '[0-9a-fA-F]{8}-(?:[0-9a-fA-F]{4}-){3}[0-9a-fA-F]{12}') { throw 'Cannot read power plan.' }
        $guid = $matches[0]
        $saved = foreach ($e in $entries) {
            $key = Get-Item -LiteralPath $e.Path -ErrorAction SilentlyContinue
            $exists = $key -and ($key.GetValueNames() -contains $e.Name)
            @{ Path = $e.Path; Name = $e.Name; Exists = [bool]$exists; Value = $(if ($exists) { $key.GetValue($e.Name) }); Kind = $(if ($exists) { [string]$key.GetValueKind($e.Name) }) }
        }
        Save-ProfileState @{ PowerPlan = $guid; Registry = @($saved) } $path
    }
    # Fail before editing capture preferences when this machine lacks this plan.
    Invoke-ProfileCommand powercfg.exe @('/setactive','8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c') | Out-Null
    foreach ($e in $entries) {
        New-Item -Path $e.Path -Force -ErrorAction Stop | Out-Null
        New-ItemProperty -LiteralPath $e.Path -Name $e.Name -Value 0 -PropertyType DWord -Force -ErrorAction Stop | Out-Null
    }
    Update-Status
    [Windows.MessageBox]::Show('High performance power plan enabled; game capture disabled. Restart the game. Power use and heat may increase. FPS improvement depends on the bottleneck.', 'FPS profile') | Out-Null
}
function Restore-FpsProfile {
    $path = Join-Path $script:profileRoot 'fps.xml'
    if (-not (Test-Path -LiteralPath $path)) { throw 'No FPS baseline saved.' }
    $s = Import-Clixml -LiteralPath $path -ErrorAction Stop
    Invoke-ProfileCommand powercfg.exe @('/setactive',$s.PowerPlan) | Out-Null
    foreach ($e in $s.Registry) {
        if ($e.Exists) {
            New-Item -Path $e.Path -Force -ErrorAction Stop | Out-Null
            New-ItemProperty -LiteralPath $e.Path -Name $e.Name -Value $e.Value -PropertyType $e.Kind -Force -ErrorAction Stop | Out-Null
        } elseif (Get-ItemProperty -LiteralPath $e.Path -Name $e.Name -ErrorAction SilentlyContinue) {
            Remove-ItemProperty -LiteralPath $e.Path -Name $e.Name -ErrorAction Stop
        }
    }
    Update-Status
}
$window.FindName('BtnSelectAdapter').Add_Click({ Select-RedproNetworkAdapter | Out-Null })
$window.FindName('BtnNetTweak').Add_Click({ Run-Tweak -Btn $this -LoadingText 'APPLYING...' -Action { Set-NetworkProfile } })
$window.FindName('BtnAdapterPreset').Add_Click({ Run-Tweak -Btn $this -LoadingText 'APPLYING...' -Action { Set-NetworkProfile -LowLatency } })
$window.FindName('BtnNetRestore').Add_Click({ Run-Tweak -Btn $this -LoadingText 'RESTORING...' -Action { Restore-NetworkProfile } })
$window.FindName('BtnNetFlush').Add_Click({ Run-Tweak -Btn $this -LoadingText 'FLUSHING...' -Action { Invoke-ProfileCommand ipconfig.exe @('/flushdns') | Out-Null } })
$window.FindName('BtnFpsBoost').Add_Click({ Run-Tweak -Btn $this -LoadingText 'APPLYING...' -Action { Set-FpsProfile } })
$window.FindName('BtnFpsRestore').Add_Click({ Run-Tweak -Btn $this -LoadingText 'RESTORING...' -Action { Restore-FpsProfile } })
$window.FindName('BtnGameSettings').Add_Click({ Start-Process 'ms-settings:gaming-gamemode' })
$window.FindName('BtnGraphicsSettings').Add_Click({ Start-Process 'ms-settings:display-advancedgraphics' })
