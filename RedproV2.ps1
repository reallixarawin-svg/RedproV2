...
$window.FindName("BtnApplyRec").Add_Click({
    Run-Tweak -Btn $window.FindName("BtnApplyRec") -LoadingText "APPLYING TWEAKS..." -Action {
        ...
        $fivemCfg = "$env:APPDATA\CitizenFX\fivem.cfg"
        if (Test-Path $fivemCfg) {
            $fivemTxt = Get-Content "$dekDir\setfivem.txt" -Raw
            Add-Content -Path $fivemCfg -Value "`n$fivemTxt" -ErrorAction SilentlyContinue
        }
        Update-Status
    }
})
