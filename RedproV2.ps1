...
<Border xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" CornerRadius="16" Background="#191E2B" BorderBrush="#7451C8" BorderThickness="1" Padding="22" Margin="0,0,0,18">
 <StackPanel><TextBlock Text="NETWORK ADAPTER" Foreground="#B79AFF" FontSize="12" FontWeight="Bold"/><TextBlock Text="ปรับการ์ดแลนตามสไตล์คุณ" Foreground="White" FontSize="23" Margin="0,6,0,8"/><TextBlock Text="โหลดค่าอัตโนมัติ • RSS ตาม CPU • ปิด Power Saving • เว้นส่วน Tweaks" Foreground="#A1AEC3" TextWrapping="Wrap" Margin="0,0,0,16"/><DockPanel Margin="0,0,0,12"><Button x:Name="BtnSelectAdapter" Content="เลือก Adapter" Height="38" Width="140" DockPanel.Dock="Left" Style="{StaticResource SecondaryButton}"/><TextBlock x:Name="TxtSelectedAdapter" Text="ยังไม่ได้เลือก Adapter" Foreground="#AEB9CC" VerticalAlignment="Center" Margin="14,0,0,0" TextTrimming="CharacterEllipsis"/></DockPanel><UniformGrid Columns="2"><Button x:Name="BtnOpenAdapter" Content="เปิด Network Tweaker  ↗" Height="44" Margin="0,0,8,0" Style="{StaticResource SecondaryButton}"/><Button x:Name="BtnAdapterPreset" Content="โหลดค่าอัตโนมัติ  →" Height="44" Margin="8,0,0,0" Style="{StaticResource PremiumButton}"/></UniformGrid><TextBlock Text="เลือก Adapter ก่อน แล้วกดโหลดค่าอัตโนมัติเพื่อเปิดเครื่องมือพร้อมค่า preset" Foreground="#A1AEC3" FontSize="11" Margin="0,12,0,0" TextWrapping="Wrap"/></StackPanel>
</Border>
"@
...
function Open-RedproNetworkAdapter {
    ...
    if ($Preset -and -not $script:selectedNetworkAdapter -and -not (Select-RedproNetworkAdapter)) { return }
    $arguments = '-NoProfile -STA -ExecutionPolicy Bypass -File "' + $path + '" -RedproLaunch'
    if ($Preset) {
        $safeAdapterName = $script:selectedNetworkAdapter.Replace('"', '\"')
        $arguments += ' -RedproPreset -SelectedAdapter "' + $safeAdapterName + '"'
    }
    try {
        $buttonLabel = if ($Preset) { 'โหลดค่าอัตโนมัติ' } else { 'เปิด Network Tweaker' }
        Write-Log "BUTTON [$buttonLabel] | STARTED"
        Start-Process -FilePath powershell.exe -ArgumentList $arguments -WindowStyle Normal -ErrorAction Stop
        Write-Log "BUTTON [$buttonLabel] | SUCCESS"
    }
    catch {
        Write-Log "BUTTON [Network Tweaker] | FAILED | $($_.Exception.Message)"
        [Windows.MessageBox]::Show($_.Exception.Message, 'Network Adapter') | Out-Null
    }
}
