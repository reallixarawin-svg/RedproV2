...
$logPath = Join-Path $logDir 'RedproV2.log'
function Write-Log {
...
function Update-LogViewer {
    ...
    try {
        $lines = if (Test-Path -LiteralPath $logPath) { @(Get-Content -LiteralPath $logPath -Tail 500 -ErrorAction Stop | Where-Object { $_ -match 'BUTTON \[' }) } else { @() }
