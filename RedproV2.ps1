...
# Prepare application views.
[xml]$appUi = $xaml
$ns = New-Object Xml.XmlNamespaceManager($appUi.NameTable)
$ns.AddNamespace('w','http://schemas.microsoft.com/winfx/2006/xaml/presentation')
$ns.AddNamespace('x','http://schemas.microsoft.com/winfx/2006/xaml')
$view = $appUi.SelectSingleNode('//*[@x:Name="ViewDashboard"]',$ns)
# Add an in-app command log view and its sidebar menu item.
$restoreNav = $appUi.SelectSingleNode('//*[@x:Name="BtnNavRestore"]',$ns)
...
$logsView = $appUi.CreateDocumentFragment()
$logsView.InnerXml = @"
...
$xaml = $appUi.OuterXml
$netNav = $appUi.SelectSingleNode('//*[@x:Name="BtnNavCPU"]',$ns).CloneNode($true)
$netNav.SetAttribute('Name','http://schemas.microsoft.com/winfx/2006/xaml','BtnNavNetwork')
$netNav.InnerXml = '<StackPanel xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" Orientation="Horizontal"><TextBlock FontFamily="Segoe MDL2 Assets" Text="&#xE701;" FontSize="16" Width="30" VerticalAlignment="Center"/><TextBlock Text="Network" VerticalAlignment="Center"/></StackPanel>'
$appUi.SelectSingleNode('//*[@x:Name="BtnNavCPU"]',$ns).ParentNode.InsertAfter($netNav,$appUi.SelectSingleNode('//*[@x:Name="BtnNavCPU"]',$ns)) | Out-Null
$netView = $appUi.CreateDocumentFragment()
$netView.InnerXml = '<Grid xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" x:Name="ViewNetwork" Visibility="Collapsed"><StackPanel><TextBlock Text="Network" FontSize="30" Foreground="White" Margin="0,0,0,18"/><Button x:Name="BtnAntiNet" Content="Redpro V2" Height="55" Margin="0,0,0,12" Style="{StaticResource PremiumButton}" FontWeight="Bold" FontSize="14"/><Button x:Name="BtnDscpFse" Content="ใช้ค่า DSCP &amp; FSE" Height="55" Style="{StaticResource PremiumButton}" FontWeight="Bold" FontSize="14"/></StackPanel></Grid>'
$appUi.SelectSingleNode('//*[@x:Name="ViewDashboard"]',$ns).ParentNode.AppendChild($netView) | Out-Null
$xaml = $appUi.OuterXml
# Read XAML
...
$views = @("ViewDashboard", "ViewCPU", "ViewInput", "ViewNetwork", "ViewPower", "ViewTools", "ViewRestore", "ViewLogs")
$navButtons = @("BtnNavDashboard", "BtnNavCPU", "BtnNavInput", "BtnNavNetwork", "BtnNavPower", "BtnNavTools", "BtnNavRestore", "BtnNavLogs")
...
$window.FindName("BtnAntiNet").Add_Click({
    Run-Tweak -Btn $window.FindName("BtnAntiNet") -LoadingText "APPLYING NETWORK..." -Action {
        $commands = @(
            ...
            'int tcp set supplemental template=internet icw=10',
            'int ip set global taskoffload=enabled',
            'interface tcp set global rsc=disabled'
        )
        foreach ($command in $commands) { & netsh $command.Split(' ') | Out-Null }
    }
})
$window.FindName("BtnDscpFse").Add_Click({
    Run-Tweak -Btn $window.FindName("BtnDscpFse") -LoadingText "APPLYING DSCP..." -Action {
        $qos = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\QoS\FiveMLag'
        New-Item -Path $qos -Force | Out-Null
        $values = @{ Version='1.0'; 'Application Name'='FiveM_GTAProcess.exe'; Protocol='*'; 'Local Port'='*'; 'Local IP'='*'; 'Local IP Prefix Length'='*'; 'Remote Port'='*'; 'Remote IP'='*'; 'Remote IP Prefix Length'='*'; 'DSCP Value'='46'; 'Throttle Rate'='1' }
        foreach ($item in $values.GetEnumerator()) { New-ItemProperty -Path $qos -Name $item.Key -Value $item.Value -PropertyType String -Force | Out-Null }
...
$window.FindName("BtnPowerImport").Add_Click({
    Run-Tweak -Btn $window.FindName("BtnPowerImport") -LoadingText "APPLYING ULTIMATE POWER..." -Action {
        $process = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$dekDir\2.Tweaker\2.Optimization Toolkit.ps1`"" -WindowStyle Hidden -Wait -PassThru -ErrorAction Stop
        if ($process.ExitCode -ne 0) { throw "Power Plan failed (exit $($process.ExitCode))." }
        Update-Status
    }
})
